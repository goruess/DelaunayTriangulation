...
function checkPointEquality(point1, point2){
  return (point1.x === point2.x) && (point1.y === point2.y);
}
function returnAdjacentSide(triangle1, triangle2){ // {a: {}, b: {}, c: {}}
  const verts1 = [triangle1.a, triangle1.b, triangle1.c];
  const verts2 = [triangle2.a, triangle2.b, triangle2.c];
  const sideCoords = []; 
  for(let i = 0; i<3; i++){   
     for(let j = 0; j<3; j++){
      if(checkPointEquality(verts1[i], verts2[j])){
        sideCoords.push(verts1[i]);
        break;
      }
    }
  }
...
function getTriangleSides(triangle){
    const side1 = [{x: triangle[0].x, y: triangle[0].y}, {x: triangle[1].x, y: triangle[1].y}];
    const side2 = [{x: triangle[0].x, y: triangle[0].y}, {x: triangle[2].x, y: triangle[2].y}];
    const side3 = [{x: triangle[1].x, y: triangle[1].y}, {x: triangle[2].x, y: triangle[2].y}];
    const sides = [];
    sides.push(side1); sides.push(side2); sides.push(side3);
    return sides;
}
